...
import com.ifg.spring.IFGApplication;
import com.ifg.spring.model.Role;
import com.ifg.spring.model.User;
import com.ifg.spring.service.RoleService;
import com.ifg.spring.service.UserService;

@RunWith(SpringJUnit4ClassRunner.class)
@SpringApplicationConfiguration(classes = IFGApplication.class)
@WebAppConfiguration
/*@SqlGroup({
    @Sql("classpath:test.sql"),  
})*/
public class IFGApplicationTest {
	private static final Logger logger = Logger.getLogger(IFGApplicationTest.class);
	@Autowired
	...
	public void createIFGOwener() {
		...
		logger.info("Creating Ifg Owner and ROLE_IFG_OWNER.");
		Role role = roleService.findByName("ROLE_IFG_OWNER");
		if (role == null) {
			logger.info("Crating Ifg Owner Role.");
			role = new Role();
			role.setId(0);
			role.setName("ROLE_IFG_OWNER");
			// roleService.saveRole(role);
		} else {
			logger.info("ROLE_IFG_OWNER Already Defined.");
		}

		Set<Role> roles = new HashSet<Role>();
		roles.add(role);
		roles.add(roleService.findByName("ROLE_IFG_OWNER"));
		user.setRoles(roles);
